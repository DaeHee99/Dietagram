...
def detect(save_img=False):
    ...
    # Food Name
    import pandas as pd
    # food_names = pd.read_csv('/Users/hyounjinlee/Downloads/03.AI알고리즘 소스코드/음식AI데이터분류.csv', header = 1)
    food_names = pd.read_csv('../../../음식AI데이터분류.csv', header = 1)
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or view_img:  # Add bbox to image
                        food_num = int(names[int(cls)])
                        if food_num in food_names.index:
                            food_name = food_names.loc[food_num, '소분류구분']
                        else:
                            food_name = 'NO DATA'
                        label = '%s %.2f' % (food_name, conf)
                        print(label)
                        plot_one_box(xyxy, im0, label=label, color=colors[int(cls)])
                        im0 = Image.fromarray(im0)
                        draw = ImageDraw.Draw(im0)
                        c1 = (int(xyxy[0]), int(xyxy[1]))
                        font = ImageFont.truetype('../../gulim.ttf', 30)
                        draw.text((c1[0], c1[1]-3), label, font = font, fill = (256, 256, 256))
                        im0 = np.array(im0)
